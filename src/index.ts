...
// Define explicit types for arXiv API XML payload
...
export async function fetchArxiv(url: string): Promise<Paper[]> {
    const now = Date.now();
    ...
    const jsonObj = parser.parse(xmlText) as { feed?: ArxivFeed };
...
function formatConciseResults(papers: Paper[]): string {
    return papers.map((p: Paper) => {
        let authorStr = p.authors.slice(0, 3).join(", ");
        if (p.authors.length > 3) authorStr += " et al.";
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === "arxiv_search") {
            ...
            const url = `${baseUrl}?search_query=${encodeURIComponent(
                query
            ...
            const url = `${baseUrl}?search_query=cat:${encodeURIComponent(
                category
            ...
            if (!paperId) throw new Error("Missing paper_id");
            ...
            const urlRelated = `${baseUrl}?search_query=cat:${encodeURIComponent(
                sourceCategory
            ...
            const filtered = relatedPapers.filter((p: Paper) => p.id !== paperId);
            ...
            const format = (args?.format as string | undefined) || "bibtex";
            const url = `${baseUrl}?id_list=${encodeURIComponent(paperId)}`;
            ...
            const paperId = args?.paper_id as string | undefined;
            const destDir = args?.destination_dir as string | undefined;

            if (!paperId || !destDir) throw new Error("Missing paper_id or destination_dir");
